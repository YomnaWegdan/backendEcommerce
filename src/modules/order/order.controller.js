...
const webHook = catchError(async (req, res, next) => {
    const stripe = new Stripe(process.env.STRIPE_SECRET_KEY)
        ...
        if(event.type !== 'checkout.session.completed') {
            await ordersModel.findByIdAndUpdate({ _id: orderId }, { status: 'rejected' });
            return res.status(400).json({ message: 'Failed' });

        }
        await ordersModel.findByIdAndUpdate({ _id: orderId }, { status: 'completed' });
...
export {createOrder , cancelOrder , webHook}
